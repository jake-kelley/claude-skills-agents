...
def main():
    ...
    if wrote_any:
        ...
        # Incremental DB indexing
        try:
            import importlib.util
            db_path = Path(__file__).parent.parent / "skills" / "second-brain" / "scripts" / "db.py"
            spec = importlib.util.spec_from_file_location("db", str(db_path))
            db = importlib.util.module_from_spec(spec)
            sys.modules["db"] = db
            spec.loader.exec_module(db)

            # Also import embed for chunking
            embed_path = Path(__file__).parent.parent / "skills" / "second-brain" / "scripts" / "embed.py"
            spec = importlib.util.spec_from_file_location("embed", str(embed_path))
            embed = importlib.util.module_from_spec(spec)
            sys.modules["embed"] = embed
            spec.loader.exec_module(embed)

            # Import text utilities
            text_path = Path(__file__).parent.parent / "skills" / "second-brain" / "scripts" / "_text.py"
            spec = importlib.util.spec_from_file_location("_text", str(text_path))
            text_mod = importlib.util.module_from_spec(spec)
            sys.modules["_text"] = text_mod
            spec.loader.exec_module(text_mod)

            conn = db.connect()
            # Re-index any notes that were just written
            for md in KB_ROOT.rglob("*.md"):
                if md.name in ("README.md", "INDEX.md"):
                    continue
                if any(part.startswith(".") for part in md.relative_to(KB_ROOT).parts):
                    continue
                parsed = text_mod.parse_frontmatter(md)
                if not parsed:
                    continue
                ...
                if old_h != h:
                    ...
                    try:
                        ...
                        log(f"kb-capture: indexed {rel_path} ({len(chunks)} chunks)")
                    except Exception as e:
                        log(f"kb-capture: index error for {rel_path}: {e!r}")
            conn.close()
        except Exception as e:
            log(f"kb-capture: DB index failed {e!r}")
