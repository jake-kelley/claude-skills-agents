...
def prompt_hook_action(hook_name: str, existing_count: int) -> str:
    ...
    print(f"\n[WARN]  Existing {hook_name} hooks detected ({existing_count} hook(s))")
    ...
    if not check_existing_hooks(settings, "UserPromptSubmit", "kb-inject.py"):
        ...
        if existing_hooks and not dry_run:
            ...
            if action == "append":
                ...
                print("   [OK] Appended kb-inject.py hook to UserPromptSubmit")
            elif action == "replace":
                settings["hooks"]["UserPromptSubmit"] = [kb_inject_hook]
                print("   [OK] Replaced UserPromptSubmit hooks with kb-inject.py")
            else:  # skip
                print("   [SKIP] Skipped UserPromptSubmit hook modification")
        else:
            # No existing hooks or dry run - just set it
            settings["hooks"]["UserPromptSubmit"] = [kb_inject_hook]
            if not dry_run:
                print("   [OK] Configured UserPromptSubmit hook")
    else:
        print("   [SKIP] kb-inject.py hook already exists in UserPromptSubmit")
    ...
    if not check_existing_hooks(settings, "Stop", "kb-capture.py"):
        ...
        if existing_hooks and not dry_run:
            ...
            if action == "append":
                ...
                print("   [OK] Appended kb-capture.py hook to Stop")
            elif action == "replace":
                settings["hooks"]["Stop"] = [kb_capture_hook]
                print("   [OK] Replaced Stop hooks with kb-capture.py")
            else:  # skip
                print("   [SKIP] Skipped Stop hook modification")
        else:
            # No existing hooks or dry run - just set it
            settings["hooks"]["Stop"] = [kb_capture_hook]
            if not dry_run:
                print("   [OK] Configured Stop hook")
    else:
        print("   [SKIP] kb-capture.py hook already exists in Stop")
...
def prompt_for_model() -> str:
    ...
    print("\n[BOT] Select model for knowledge base operations:")
    ...
    while True:
        ...
        if choice in models:
            ...
            if choice == "4":
                ...
                if custom_id:
                    print(f"   [OK] Selected: {custom_id}")
                    return custom_id
                else:
                    print("   Invalid model ID. Please try again.")
                    continue
            print(f"   [OK] Selected: {description}")
...
def create_storage_tree(claude_dir: Path, source_dir: Path, dry_run: bool = False, model_id: Optional[str] = None) -> None:
    ...
    if not config_dst.exists() or dry_run:
        ...
        if not dry_run and model_id:
            ...
            print(f"   [OK] Configured models: {model_id}")
...
def verify_installation(claude_dir: Path) -> bool:
    ...
    for path in required_paths:
        if path.exists():
            print(f"  [OK] {path.relative_to(claude_dir)}")
        else:
            print(f"  [FAIL] MISSING: {path.relative_to(claude_dir)}")
            all_ok = False
    
    return all_ok


def install_python_deps(dry_run: bool = False) -> bool:
    """Install required Python packages. Returns True if successful."""
    deps = ["sqlite-vec>=0.1.6", "fastembed>=0.3.0"]
    print("\n[PACKAGE] Installing Python dependencies...")
    if dry_run:
        print(f"  [DRY-RUN] Would run: pip install {' '.join(deps)}")
        return True
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"  [OK] Installed: {', '.join(deps)}")
            return True
        else:
            print(f"  [WARN]  pip install failed (returncode {result.returncode})")
            if result.stderr:
                print(f"     {result.stderr[:200]}")
            print(f"     You can install manually: pip install {' '.join(deps)}")
            return False
    except subprocess.TimeoutExpired:
        print("  [WARN]  pip install timed out")
        return False
    except Exception as e:
        print(f"  [WARN]  pip install error: {e}")
        return False


def run_initial_reindex(claude_dir: Path, source_dir: Path, dry_run: bool = False) -> None:
    """Build the initial vector DB from existing notes."""
    kb_root = claude_dir / "second-brain"
    db_file = kb_root / ".db" / "index.sqlite"

    # Check if DB already exists
    if db_file.exists():
        print("\n[SYNC] Vector DB already exists — skipping reindex.")
        return

    print("\n[SYNC] Building initial vector index from existing notes...")
    if dry_run:
        print("  [DRY-RUN] Would run reindex.py")
        return

    script = source_dir / "skills" / "second-brain" / "scripts" / "reindex.py"
    if not script.exists():
        print(f"  [WARN]  reindex.py not found at {script}")
        return

    try:
        env = dict(os.environ)
        env["KB_CAPTURE_RUNNING"] = "1"
        subprocess.run([sys.executable, str(script)], env=env, timeout=300, check=False)
        print("  [OK] Vector DB initialized")
    except subprocess.TimeoutExpired:
        print("  [WARN]  reindex timed out")
    except Exception as e:
...
2. Test the skill:
   /second-brain status      # Check installation and vector DB stats
...
4. The system works automatically:
   ...
   - Before each prompt: relevant notes are injected via hybrid vector+FTS search
...
[SPEED] First run:
   - The embedding model (~30MB) will download on first query/prompt (cached in ~/.cache/fastembed/)
   - This is completely offline and only happens once
   - Subsequent queries use the cached model (~50ms cold start)
...
""")
    ...
    print("[DEPLOY] Second Brain Skill Deployment")
    print("=" * 50)
    ...
    print(f"\n[LOCATION] Claude directory: {claude_dir}")
    print(f"[PACKAGE] Source directory: {source_dir}")
    
    # Verify source files exist
    manifest_path = source_dir / "manifest.json"
    if not manifest_path.exists():
        print(f"\n[ERR] Error: manifest.json not found in {source_dir}")
        print("   Make sure you're running install.py from the deployment package directory.")
        return 1
    
    # Create base directories
    print("\n[DIR] Creating base directories...")
    ...
    try:
        ...
        # Step 2: Install hooks, agents, skills
        ...
        print("\n[CONFIG]  Updating settings.json...")
        update_settings_json(claude_dir, source_dir, args.dry_run)

        # Step 4: Build initial vector DB
        run_initial_reindex(claude_dir, source_dir, args.dry_run)
        
    except Exception as e:
        print(f"\n[ERR] Error during installation: {e}")
        import traceback
        traceback.print_exc()
        return 1
    ...
    if not args.dry_run:
        if verify_installation(claude_dir):
            ...
            print("\n[WARN]  Some components may not have installed correctly.")
            return 1
    else:
