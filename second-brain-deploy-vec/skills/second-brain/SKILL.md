...
|---|---|
| `query <q>` or no subcommand with text | Run `python <SKILL_DIR>/scripts/query.py "<q>"` — hybrid vector+FTS search, returns top matches with paths and excerpts |
...
| `consolidate` | Run `python <SKILL_DIR>/scripts/consolidate.py` — regenerates INDEX, rebuilds metadata, syncs vector DB, reports near-duplicates |
| `reindex [--force]` | Run `python <SKILL_DIR>/scripts/reindex.py` — rebuild the vector DB from the Markdown source-of-truth. Use `--force` for full rebuild after schema changes. |
...
| `config --set-model <op> <model>` | Set the model for a specific operation (extractor, ingest, embedder). E.g. `config --set-model extractor claude-opus-4-7` |
| `status` | Show counts (notes per category), vector DB stats, publish target, auto-capture toggle |
| `help` or (no args) | Show detailed help (all subcommands, examples, how it works) |

## Architecture

**Markdown files are the source of truth.** The SQLite vector database at `~/.claude/second-brain/.db/index.sqlite` is a derived cache that can be rebuilt at any time via `reindex.py` or `consolidate`.

**Retrieval is hybrid:** queries use both BM25 full-text search (FTS5) and vector semantic search (via local BAAI/bge-small-en-v1.5 embeddings), merged with Reciprocal Rank Fusion. This gives both exact-match precision and semantic recall.

**Embeddings are fully offline:** the bge-small model (~30MB) downloads on first use to `~/.cache/fastembed/` and is then cached locally. No API keys, no cloud calls.
...
├── .db/                (generated) vector database & cache
│   └── index.sqlite    sqlite-vec + FTS5 indexes
├── .metadata/          (generated) hashes, tags indices
